...
checkInitialKind ki
  ...
  = do { case m_sig of
           Nothing -> return ()
           Just sig -> failWithTc (kindAnnErr (ppr sig))
       ; tc <- kcDeclHeader name (newOrDataToFlavour new_or_data) ki ktvs
...
checkInitialKind ki
    ...
    (FamilyDecl { fdLName     = (dL->L _ name)
                ...
                , fdInfo      = info })})
  = do { case resultSig of
           NoSig{} -> return ()
           KindSig _ sig -> failWithTc (kindAnnErr (ppr sig))
           TyVarSig _ (unLoc -> tv) ->
             case tv of
               UserTyVar{} -> return ()
               KindedTyVar _ _ ki -> failWithTc (kindAnnErr (ppr ki))
...
checkInitialKind ki
    (SynDecl { tcdLName = dL->L _ name
             , tcdTyVars = ktvs })
  = do  { tycon <- kcDeclHeader name TypeSynonymFlavour ki ktvs
        ; return [tycon] }
...
kcDeclHeader :: Name
             ...
             -> LHsQTyVars GhcRn
             -> TcM TcTyCon
kcDeclHeader name flav ki ktvs =
  ...
  where
    ...
    go d_ki tcb_acc bndrs =
      case tcSplitPiTy_maybe d_ki of
        ...
        Just (Anon InvisArg _, d_ki') ->
          -- TODO (int-index): make use of the binder?
          go d_ki' tcb_acc bndrs
        Just (Anon VisArg bndr_ki, d_ki') ->
          case bndrs of
            L _ (UserTyVar _ v):bs ->
              do { tcv <- newSkolemTyVar (unLoc v) bndr_ki
                 ; let tcb = mkAnonTyConBinder VisArg tcv
                 ; tcExtendTyVarEnv [tcv] $
                   go d_ki' (tcb:tcb_acc) bs
                 }
            L _ (KindedTyVar _ v v_hs_ki):bs ->
            ...
            L _ (UserTyVar _ v):bs ->
              do { tcv <- newSkolemTyVar (Var.varName v') (varType v')
                 ...
                 ; tcExtendNameTyVarEnv [(tyVarName tcv, tcv), (unLoc v, tcv)] $
                   go d_ki' (tcb:tcb_acc) bs }
            b@(L _ KindedTyVar{}):_ -> failWithTc (kindAnnErr (ppr b))
            L _ (XTyVarBndr x):_ -> absurd x
            [] -> done d_ki tcb_acc
        Nothing ->
          case bndrs of
            _:_ -> failWithTc $ text "Too many binders for a TLKS"
            [] -> done d_ki tcb_acc
    done d_ki tcb_acc =
      let tcbs = reverse tcb_acc
          all_tv_prs = mkTyVarNamePairs (map binderVar tcbs)
      in return $ mkTcTyCon name tcbs d_ki all_tv_prs True flav


kindAnnErr :: SDoc -> SDoc
kindAnnErr thing =
  hang (text "Kind annotation not allowed for a declaration with a TLKS:")
    2 thing
